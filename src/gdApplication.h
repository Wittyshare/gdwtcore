...
public :
  ...
  std::string            getUserRootAppPath(bool bUseSlash = true) {
    return getUserRootAppPath("", bUseSlash);
  };
  ...
  static gdApplication*  gdinstance() {
    return (gdApplication*) instance();
  };

  /*! \brief Check if the application run on the local network  */
  bool                   isOnInternalNet() {
    return m_bRemoteUser;
  };
