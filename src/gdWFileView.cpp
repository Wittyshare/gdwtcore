...
void gdWFileView::init(const boost::filesystem::path& startPath)
 {
  ...
  if ( m_tbFlags & tbUseLeftTree )
     ...
     if ( !loadTree(m_pModelView->invisibleRootItem(), startPath, noSkipParent) ) return;;
...
void gdWFileView::doDelete(Wt::StandardButton button)
 {
  ...
  char type = str[0];
  str.erase(0, 1);
  try { boost::filesystem::remove_all(str);
   } catch (boost::filesystem::filesystem_error& e) {
     std::cerr << e.what() << std::endl;
     return;
   }
  ...
  if ( !bOnRename )
    {
     ...
     try { boost::filesystem::create_directory(newPath);
      } catch (boost::filesystem::filesystem_error& e) {
        std::cerr << e.what() << std::endl;
        return;
      }
   ...
   else
    {
     ...
     try { boost::filesystem::rename(oldPath, newPath);
      } catch (boost::filesystem::filesystem_error& e) {
        std::cerr << e.what() << std::endl;
        return;
      }
