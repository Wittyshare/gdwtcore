...
void gdWForm::doLdap()
{
  ...
  gdCLdapEntry       entry;
  gdCLdapAttribute*  newAttr01 = new gdCLdapAttribute("objectClass");
  newAttr01->addValue("EurIndividualPerson");
  ...
  for (int cpt = 0; cpt < m_vObjects.size(); cpt++) {
    ...
    gdCLdapAttribute*  newAttr = new gdCLdapAttribute(m_vObjects[cpt]->m_outputAttribute);
    newAttr->addValue(objectValues(m_vObjects[cpt]->m_pObject));
    newAttr->bNewAttribute = true;
    entry.Attrs.push_back(newAttr);
  }
  gdCLdapAttribute*  newAttr = new gdCLdapAttribute("cn");
  newAttr->addValue(sSn + " " + sGivenName);
  newAttr->bNewAttribute = true;
  entry.Attrs.push_back(newAttr);
